...
def create_text_thumbnail(text, filename_prefix):
    try:
        ...
        if not lines: lines = ["농어업재해 손해평가사 자격증", "핵심 실무 가이드"]
...
def generate_post(campaign, keyword):
    ...
    draft_prompt = f"""당신은 공공 제도 및 정책 지원 요건을 정확하고 객관적으로 전달하는 전문 정보 가이드 에디터입니다.
어떠한 가짜 신분(예: "15년 차 법률상담관", "전문가" 등)도 사칭하지 마세요.
...
{{
  ...
  "thumb_hook": "{keyword}\\n핵심 실무 가이드 (줄바꿈 \\n)",
  "vibe_keywords": "study agriculture exam",
...
---METADATA_END---"""
    ...
    vibe_keywords = meta.get('vibe_keywords', 'study agriculture exam')
